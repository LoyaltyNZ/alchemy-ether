...
class Service
  ...
  constructor: (@name, options = {}, @service_fn = ( (msg) -> {})) ->
    ...
    binding_keys = @options.resource_paths.map( (path) -> "#{Service.path_to_routing_key(path)}.#")
  ...
  send_message_to_service: (service_name, message) ->
    @send_HTTP_message('', service_name, message)
  ...
  send_message_to_resource: (message) ->
    routing_key = Service.path_to_routing_key(message['path'])
    @send_HTTP_message('resources.exchange', routing_key, message)
  ...
  send_request_to_service: (service_name, message) ->
    @send_HTTP_request('', service_name, message)
  ...
  #
  # This method can optionally take a block which will be executed asynchronously and yielded the response
  send_request_to_resource: (message) ->
    routing_key = Service.path_to_routing_key(message['path'])
    @send_HTTP_request('resources.exchange', routing_key, message)



  # `send_HTTP_message` send http message
  #
  #
  ...
  send_HTTP_request: (exchange, queue, message) ->
    http_message = {
  ...
  process_response_queue_message: (msg) =>
    ...
    if not deferred? or msg.properties.type != 'http_response'
      ...
      return
    deferred.resolve_promise(JSON.parse(msg.content))
...
class ServiceConnectionManager
  ...
  start: ->
    ...
    try
      @get_service_channel()
      .then( (channel) =>
        #bind the binding keys to the resources exchange
        channel.assertExchange("resources.exchange", 'topic')
        .then( =>
          promises = []
          for bk in @resources_binding_keys
            promises.push channel.bindQueue(@service_queue_name, "resources.exchange", bk)
          bb.all(promises)
        )
      )
      .then( =>
